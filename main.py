    ...
    if ('user' in session and session['user'] == params['admin-username']):
        if request.method == 'POST':
            ...
            if (sno == '0'):
                ...
                db.session.add(post)
                db.session.commit()
                flash("Thanks for uploading this amazing post", "success")
                ...
                post.date = date
                db.session.commit()
                flash("You edited this post, thanks for the contribution", "success")
...
def uploader():
    if ('user' in session and session['user'] == params['admin-username']):
        if (request.method == 'POST'):
            ...
            flash("your image upoloaded successfully", "success")
...
def delete(sno):
    if ('user' in session and session['user'] == params['admin-username']):
        ...
        db.session.commit()
        flash("you have deleted this post", "success")
    return redirect('/dashboard')
...
def contact():
    if(request.method=='POST'):
        ...
        mail.send_message("Dolton's Social blog ! a new essage from " + name,
                          sender = email,
                          recipients = [params['gmail-user']],
                          body = message + "\n" + " Please contact me on " + phone + "\n" + "\n" + "Best Regards," + "\n" +"Dolton's Social Team")
